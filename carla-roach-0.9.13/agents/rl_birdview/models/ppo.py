...
class PPO():
    ...
    def learn(self, total_timesteps, callback=None, seed=2021):
        ...
        while self.num_timesteps < total_timesteps:
            ...
            t0 = time.time()
            print("before policy.train()", type(self.policy))
            self.policy = self.policy.train()
            print("after policy.train()")
            continue_training = self.collect_rollouts(self.env, callback, self.buffer, self.n_steps)
            self.t_rollout = time.time() - t0
            callback.on_rollout_end()
            
            image_list = self.buffer.render()
            print("image list length ", len(image_list), image_list[0].shape, type(image_list[0]), image_list[0].dtype)
            # self.create_gif(image_list, "ppo.gif")
            if continue_training is False:
                break

            t0 = time.time()
            self.train()
            logging.info(self.train_debug)
            self.t_train = time.time() - t0
            callback.on_training_end()

        return self
    
    def create_gif(self, image_list, output_path):
        """
        ...
        output_path = os.path.join(r"C:\Users\abjawad\Documents\GitHub\carla-roach-0.9.13\outputs", output_path)
        print("output path ", output_path)
        with imageio.get_writer(output_path, mode='I', duration=2) as writer:
