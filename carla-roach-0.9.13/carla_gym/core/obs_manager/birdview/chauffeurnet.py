...
class ObsManager_Base(ObsManagerBase):
    def __init__(self, obs_configs):
        ...
        super(ObsManager_Base, self).__init__()
    ...
    def _get_mask_from_stopline_vtx(self, stopline_vtx, M_warp):
        ...
        for sp_locs in stopline_vtx:
            ...
            # Ensuring the points are tuples of integers
            pt1 = tuple(int(coord) for coord in stopline_warped[0, 0])
            pt2 = tuple(int(coord) for coord in stopline_warped[1, 0])
    ...
    def set_spectator(self, carla_vehicle):
        import math
        # Get the current location and rotation of the vehicle
        spec_location = carla_vehicle.get_location()
        spec_rotation = carla_vehicle.get_transform().rotation

        # Adjust the spectator location for a third-person view
        # Move the spectator up (z) and slightly back (x) relative to the vehicle's forward direction
        spec_location.z += 5  # Move up to get a better aerial view
        spec_rotation.pitch = -15  # Tilt the camera downwards

        # You may need to adjust these offsets based on your specific requirement
        offset_distance = 10  # Distance behind the car. Adjust as needed for a better view.
        
        # Calculate the offset in the x-direction (forward of the vehicle)
        # We use trigonometry to calculate backward movement from the vehicle's forward facing direction.
        spec_location.x += -offset_distance * math.cos(math.radians(spec_rotation.yaw))
        spec_location.y += -offset_distance * math.sin(math.radians(spec_rotation.yaw))
        
        # Reset roll to 0 as we typically don't want the spectator view rolling
        spec_rotation.roll = 0
        
        # Set the transformed spectator location and rotation
