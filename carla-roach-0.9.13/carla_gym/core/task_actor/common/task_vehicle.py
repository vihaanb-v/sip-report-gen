...
class TaskVehicle(object):
    def __init__(self, vehicle, target_transforms, spawn_transforms, endless):
        ...
        if len(self._target_transforms) == 0:
            ...
            for cnt in range(500):
                # print(self._route_length)
                # while (
                #     self._route_length < 2
                # ):  # the route length was defined 1000 meter or 1 kilo meter, but we do not need such long route.
                #     print(self._route_length)
    ...
    def _add_random_target(self):
        if len(self._target_transforms) == 0:
            ...
            last_target_transform = self.vehicle.get_transform()
            ...
            last_target_transform = self._target_transforms[-1]
            new_target_transform = np.random.choice(
                [x[1] for x in self._spawn_transforms if x[0] != last_road_id]
            )

        self._add_specific_target(last_target_transform, new_target_transform)
        
    def _add_specific_target(self, origin, destination):
        try:
            route_trace = self._planner.trace_route(
                origin.location, destination.location
            )
            self._global_route += route_trace
            self._target_transforms.append(destination)
